...
- **🔄 GitOps Native** - ArgoCD integration, Git as single source of truth
- **☸️ Kubernetes Operator** - Cloud-native architecture using CRDs
- **🏗️ Infrastructure as Code** - Terraform-based deployments
- **📊 Real-time Monitoring** - Built-in observability and cost tracking
- **🔐 Self-Hosted** - Deploy on your infrastructure or use our managed service
- **📈 Auto-scaling** - Intelligent scaling based on metrics and schedules
- **🔄 Infrastructure Cloning** - One-click environment duplication
...
│                    Grid Platform                            │
├─────────────────────────────────────────────────────────────┤
│  grid-ui (React)     grid-core (Node.js)    grid-terraform │
│  ┌─────────────┐     ┌─────────────────┐    ┌─────────────┐ │
│  │ Dashboard   │◄──►│ REST API        │◄──►│ Terraform   │ │
│  │ Deployments │     │ WebSocket       │    │ Modules     │ │
│  │ Monitoring  │     │ Auth & RBAC     │    │ Multi-Cloud │ │
│  └─────────────┘     └─────────────────┘    └─────────────┘ │
├─────────────────────────────────────────────────────────────┤
│  grid-operator (Go)           grid-docs (Docusaurus)        │
│  ┌─────────────────┐          ┌─────────────────────────┐   │
│  │ CRDs & Controllers│        │ Documentation          │   │
│  │ GitOps Integration│        │ API Reference          │   │
│  │ Multi-Cloud     │          │ Tutorials & Guides     │   │
│  └─────────────────┘          └─────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
| **Multi-Cloud** | ✅ GCP, AWS, Azure | ✅ All clouds | ❌ AWS only | **✅ GCP, AWS, Azure** |
| **GitOps** | ⚠️ Basic | ⚠️ VCS integration | ❌ Limited | **✅ ArgoCD native** |
| **Extensibility** | ❌ Closed | ⚠️ Provider API | ❌ Closed | **✅ Open source** |
| **Kubernetes Native** | ❌ | ❌ | ⚠️ Partial | **✅ CRDs + Operator** |
